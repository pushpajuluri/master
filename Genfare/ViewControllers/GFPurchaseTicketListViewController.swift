...
class GFPurchaseTicketListViewController:UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    var products:Array<Product> =  GFDataService.GetProducts()!
    ...
    override func viewDidLoad() {
        ...
        let payAsYouGoText =  returnStoredValueWithActivation()
    ...
    func targetMethod(){
        ...
        if products.count > 0{
            let  filteredProdcutArray = returnStoredValueProducts()
    ...
    func payasyougo(){
        if products.count > 0 {
            ...
            let filteredProductArrayPayAsYouGo  = returnStoredValueWithActivation()
            ...
            for i in filteredProductArrayPayAsYouGo{
                if(!(self.PayAsYouGoTextField.text?.isEmpty)!){
                    ...
                    dict["ticketTypeDescription"] = i.ticketTypeDescription
                    dict["ticket_count"] = 0
                    var payasyougoTextfield = self.PayAsYouGoTextField.text
    ...
    @IBAction func firstPageContinueButtonClicked(_ sender: GFMenuButton) {
        ...
        for j in 0..<productsListArray.count {
            let count = productsListArray[j]["ticket_count"] as? Int
            if !(count == 0) {
